...
#checks input was formatted correctly
if [[ ! $INPUT_DATE =~ ^[0-9]{8}$ ]]; then
	echo "Uh oh: I don't like your input >:( Use the format YYYYMMDD"
	exit 1
fi
#checks if date will have data
YESTERDAY=$(date -d "yesterday" +"%Y%m%d")
if [[ $INPUT_DATE -ge $YESTERDAY ]]; then 
	echo "There is no data for this day yet. Early bird doesn't always get the worm"
	exit 1
fi 
YEAR=${INPUT_DATE:0:4}
...
more LOGS/Temps/*.CSV
read -p "Beautiful Temperature Data? (YES/NO): " DATA_CHECK
if [[ $DATA_CHECK != "YES" ]]; then 
	echo "Data check failed. Abort mission."
	exit 1
fi 
#confirm PID data
...
more LOGS/PID_Data/*.CSV
read -p "Beautiful PID Data? (YES/NO): " PID_CHECK
if [[ $PID_CHECK != "YES" ]]; then
	echo "Data check failed. Abort mission."
	exit 1
fi 
echo "Yay pretty Data! Let's continue :)"
